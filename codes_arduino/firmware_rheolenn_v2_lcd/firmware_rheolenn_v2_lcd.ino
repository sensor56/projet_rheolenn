...
const int nombreCirculateurs=1; // nombre de circulateurs - 4 max
int broches_circulateur[nombreCirculateurs]={7}; // broches des circulateurs - dispo sur broches 4 à 7 
int etatCirculateur[nombreCirculateurs]={LOW};
...
int flagAuto=true; // variable pour regulation auto
...
void setup() {
  ...
  Serial.println("----- Rheolenn v2 -----"); // message initial
...
void loop() {
    ...
    if (chaineReception!="") { // si une chaine a été reçue
       ...
       if (chaineReception=="getStatus()")getStatus(); // forme simple
       else if(utils.testInstructionLong(chaineReception,"getTemp(",1,params)==true) Serial.println(getTemp(params[0])); 
       else if(utils.testInstructionLong(chaineReception,"setAuto(",1,params)==true) flagAuto=params[0], Serial.println(flagAuto); 
       ...
       //else if (chaineReception=="initSensor()") initSensor(0); // forme simple - la fonction realise elle-meme son affichage serie
        ...
        else if(utils.testInstructionLong(chaineReception,"setDebug(",1,params)==true) debug=params[0], Serial.println(debug); 
        
        else Serial.println(""); // dans tous les cas on renvoie un saut de ligne de validation
...
//---- fonction status ---- renvoie ensemble valeurs à un instant : TBa | TPa | Diff | Circ
void getStatus(){
  
     // temperatures 
     for (int i=0; i<nombreOnewire; i++) { 
         Serial.print(temperatures[i]), Serial.print("|");   //pas de if debug car c'est le job de la fonction..       
    } // fin for
  
    Serial.print(deltaCapteurs), Serial.print("|");
    
    Serial.print(etatCirculateur[0]), Serial.println("|");    
...
void mesureCapteurs() {
    ...
    //for (int i=0; i<sizeof(capteurs)/2; i++) { 
     for (int i=0; i<nombreOnewire; i++) { 
         ...
         if (debug) Serial.println(temperatures[i]); 
...
void regulation(int indexIn){
    ...
    if (debug) Serial.print("Diff="), Serial.print(deltaCapteurs), Serial.println("C"); 
    ...
    if (debug) Serial.print("Etat Circulateur =");
    if (debug) if (etatCirculateur[indexIn]) Serial.println("ON"); else Serial.println("OFF");
...
//XXXXXXXXXXXXXXXXXXXX Fonctions LCD XXXXXXXXXXXXXXXXXXXXXXXXXXXX
void refreshLCD(){

     // affiche valeur temp  
     for (int i=0; i<nombreOnewire; i++) { 
       
         lcd.setCursor(0,i); // se place ligne voulue
         if (i==0)lcd.print("TBa:"), lcd.print(temperatures[i]);
         if (i==1)lcd.print("TPa:"), lcd.print(temperatures[i]);

    } // fin for

    lcd.setCursor(0,2); // se place ligne voulue
    lcd.print("Diff:"), lcd.print(deltaCapteurs),lcd.print("  ");

    // etat circulateur 
    for (int i=0; i<nombreCirculateurs; i++) { 
      lcd.setCursor(i*4,3); // se place ligne voulue
      if (etatCirculateur[i]) lcd.print("ON "); else lcd.print("OFF");
    }
